...
function printFileListing($path = "./", $pattern = "*", $excluded = array(), $sort = "n", $direction = SORT_ASC) {
	$files = glob("$path$pattern");
        ...
        $rootfiles = glob("$path*.root");
        $files = array_merge($files, $dirlist);
	...
	//echo "<p><a href=\"" . parentDirectory() . "\">&larr; Parent directory</a></p>\n";
        echo "<p><a href='http://atlaswww.hep.anl.gov/hepsim/'>&larr; back to HepSim</a></p>\n";
	echo "<table>\n";
	echo "\t<tr>\n\t\t<th></th>\n";
	...
	foreach ($files as $index => $file) {
		...
		$class    = $class == "even" ? "odd" : "even";
	        if (!is_dir($file)) { $tot_size = $tot_size + $sizes[$index]; }
                $nev=$nev+1;
		echo "\t<tr class=\"$class\">\n" .
                         "\t\t<td>" . $nev . "</td>\n" .	
         ...
         $TPP=$tot_size /(1024*1024*1024);
         $TFF=number_format($TPP, 3) . " GB" ;
         echo "\t<tr class=\"$class\">\n" .
                         "\t\t<td>  </td>\n" .
                         "\t\t<td>  </td>\n" .
                         "\t\t<td>  </td>\n" .
                         "\t\t<td> <small>$TFF</small> </td>\n" .
                         "\t\t<td> </td>\n" .
                         "\t</tr>\n";

	echo "</table>\n";
        //echo "Total size=" . number_format($TPP, 2) . " GB";
}
